...
System.Collections.ObjectModel.ReadOnlyCollection
        ...
        {
            HResult hr = pfops == null ? throw new ArgumentNullException(nameof(pfops)) : IsDisposed ? throw new ObjectDisposedException(nameof(FileOperation)) : _fileOperation.Advise(pfops
#if !WAPICP3
        ...
        {
            HResult hr = IsDisposed ? throw new ObjectDisposedException(nameof(FileOperation)) : _fileOperation.Unadvise(cookie);
       ...
       fileOperationProgressSink) => GetHResult(() => _fileOperation.CopyItem(item, destinationFolder, copyName, fileOperationProgressSink
            ...
            EmptyRecycleBinFlags emptyRecycleBinFlags)
        {
            ...
            switch (hr)
            {
                case HResult.Ok: return true;

                case HResult.Fail: return false;
            }
    ...
    public delegate HResult PostNewItem(TransferSourceFlags dwFlags, IShellItem psiDestinationFolder, string pszNewName, string pszTemplateName, FileAttributes dwFileAttributes, HResult hrNew, IShellItem psiNewItem);
