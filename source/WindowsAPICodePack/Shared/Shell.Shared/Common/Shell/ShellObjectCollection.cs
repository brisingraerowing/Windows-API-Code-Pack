...
namespace Microsoft.WindowsAPICodePack.Shell
{
    ...
    public class ShellObjectCollection : IEnumerable, IDisposable, IList<ShellObject>
    {
        ...
        {
            var iid = new Guid(NativeAPI.Guids.Shell.IShellItemArray);
        ...
        {
            var mstream = content.Count == 0 ? throw new InvalidOperationException(LocalizedMessages.ShellObjectCollectionEmptyCollection) : new MemoryStream();
            ...
            try
            {
                using (var bwriter = new BinaryWriter(mstream))
                {
                    ...
                    for (int index = 0; index < itemCount; index++)
                        ...
                        idls[index] = index == 0 ? ((ShellObject)KnownFolders.Desktop).PIDL : idls[index] = content[index - 1].PIDL;
        ...
        public void Insert(int index, ShellObject item) => (IsReadOnly ? throw new InvalidOperationException(LocalizedMessages.ShellObjectCollectionInsertReadOnly) : content).Insert(index, item);

        /// <summary>
        /// Removes the specified ShellObject from the collection
        /// </summary>
        /// <param name="index">The index to remove at.</param>
        public void RemoveAt(int index) => (IsReadOnly ? throw new InvalidOperationException(LocalizedMessages.ShellObjectCollectionRemoveReadOnly) : content).RemoveAt(index);
        ...
        {
            get => content[index];
            set => content[index] = IsReadOnly ? throw new InvalidOperationException(LocalizedMessages.ShellObjectCollectionInsertReadOnly) : value;
        }
        #endregion
        ...
        public void Add(ShellObject item) => (IsReadOnly ? throw new InvalidOperationException(LocalizedMessages.ShellObjectCollectionInsertReadOnly) : content).Add(item);

        /// <summary>
        /// Clears the collection of ShellObjects.
        /// </summary>
        public void Clear() => (IsReadOnly ? throw new InvalidOperationException(LocalizedMessages.ShellObjectCollectionRemoveReadOnly) : content).Clear();
        ...
        {
            for (int index = array == null ? throw new ArgumentNullException(nameof(array)) : array.Length < arrayIndex + content.Count ? throw new ArgumentException(LocalizedMessages.ShellObjectCollectionArrayTooSmall, nameof(array)) : 0; index < content.Count; index++)
